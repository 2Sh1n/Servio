...
const ProviderDetails = () => {
  ...
  const [targetServicesCount, setTargetServicesCount] = useState(5);
  ...
  const panelBaseUrl =
    process.env.NEXT_PUBLIC_PANEL_URL || "http://localhost/servio/apps/serviopanel";
  const backendAssetsBase = `${panelBaseUrl}/public/backend/assets`;
  const defaultImage = `${backendAssetsBase}/default.png`;

  const resolveAssetUrl = (folder, value) => {
    ...
    const cleanValue = fileName.replace(/^\/+/, "");

    if (cleanValue.includes("public/backend/assets/")) {
      return `${panelBaseUrl}/${cleanValue}`;
    }

    if (cleanValue.startsWith("backend/assets/")) {
      return `${panelBaseUrl}/public/${cleanValue}`;
    }

    return `${backendAssetsBase}/${folder}/${cleanValue}`;
  };

  const normalizeOtherImages = (rawImages) => {
    if (!rawImages) return [];
  ...
  const providerBannerUrl = useMemo(() => {
    return resolveAssetUrl("banner", providerData?.banner_image);
  }, [providerData]);

  const providerProfileUrl = useMemo(() => {
    return resolveAssetUrl("profile", providerData?.image);
  }, [providerData]);

  const providerOtherImages = useMemo(() => {
    return normalizeOtherImages(providerData?.other_images).map((image) =>
      resolveAssetUrl("profiles", image)
    );
  }, [providerData]);
  ...
  useEffect(() => {
    ...
    setIsOpenInApp(Boolean(isShare && isMobileOrTablet));
  }, [isShare]);
  ...
  if (isErrorProvider || !providerData) {
    return (
      <Layout>
        <BreadCrumb
              ...
              <div className="flex flex-col gap-4">
                <div className="rounded-[18px] provider_info_card dark:card_bg shadow-sm border border-gray-200">
                  <div className="overflow-hidden p-6 pb-0">
                    <img
                      src={providerBannerUrl}
                      alt={providerData?.company_name || "banner"}
                      className="w-full aspect-provider-banner object-cover rounded-xl"
                  ...
                  <div className="p-5">
                    <div className="flex flex-col sm:flex-row items-start gap-3">
                      <div className="w-14 h-14 rounded-xl overflow-hidden border border-gray-100">
                        <img
                          src={providerProfileUrl}
                          alt={providerData?.company_name || "profile"}
                  ...
                  <div className="light_bg_color rounded-lg overflow-hidden mt-4 relative p-5">
                    <div>
                      ...
                      <div className="photos grid grid-cols-3 gap-4 mt-6">
                        {providerOtherImages.slice(0, 4).map((image, index) => (
                          <div
                          ...
                          >
                            <img
                              ...
                              className="rounded-md w-full aspect-service-other object-cover"
                              onError={handleImgError}
                            />
                          </div>
                        ))}

                        {providerOtherImages.length >= 5 && (
                          <div className="photo col-span-2 cursor-pointer">
                            ...
                            >
                              <img
                                src={providerOtherImages[4]}
                                alt={providerData?.company_name || "gallery"}
                                className="w-full aspect-service-other2 object-cover"
                                onError={handleImgError}
                              />
                              ...
                              {providerOtherImages.length > 5 && (
                                <div className="absolute inset-0 bg-gray-900 bg-opacity-70 flex justify-center items-center">
                                  <span className="text-md font-bold text-white">
                                    +{providerOtherImages.length - 5} {t("more")}
                                  </span>
                                </div>
                              )}
                            </div>
