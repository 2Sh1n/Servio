...
function fetch_cart($from_app = false, int $user_id = 0, string $search = '', $limit = 0, int $offset = 0, string $sort = 'c.id', string $order = 'Desc', $where = [], $additional_data = [], $reorder = null, $order_id = null)
{
    ...
    if ($from_app) {
        ...
        if (!empty($service_record)) {
            ...
            $data['sub_total'] = number_format(($sub_total + $tax_total), 2, '.', '');
            ...
            // Format overall amount without comma separators but keep decimal point
            // overall_amount = subtotal + tax + visiting charges
            $data['overall_amount'] = number_format(
                $sub_total + $tax_total + $data['visiting_charges'],
                2,
                '.',
                ''
            );
...
function queue_notification_service(string $eventType, array $recipients = [], array $context = [], array $options = [], string $priority = 'default'): mixed
{
    ...
    if (!empty($allJobIds) && !empty($options['batch_id'])) {
        try {
            $trackingDb = \Config\Database::connect();
            $trackingDb->table('notification_dispatches')->insert([
                'batch_id'         => $options['batch_id'],
                'title'            => mb_substr($dispatchTitle, 0, 255),
                'event_type'       => $eventType,
                'channels'         => implode(',', $queuedChannels),
...
function booked_timings($partner_id, $date_of_service)
{
    ...
    $response = $table->select('o.starting_time,o.ending_time')
        ...
        ->whereIn('o.status', ['confirmed', 'rescheduled', 'awaiting', 'started'])
        ->groupBy('o.id')
...
function check_availability($partner_id, $booking_date, $time)
{
    ...
    if (isset($timings) && !empty($timings)) {
        ...
        $booked_slots = $table->select('a.starting_time AS free_before, (a.starting_time + INTERVAL a.duration HOUR_MINUTE) AS free_after')
            ...
            ->whereIn('status', ['awaiting', 'pending', 'confirmed', 'rescheduled', 'started'])
            ->where('partner_id', '50')
            ->groupBy('id')
    ...
    if (!empty($next_day_order)) {
        ...
        if (isset($timings) && !empty($timings)) {
            ...
            if (isset($booked_slots) && !empty($booked_slots)) {
                ...
                $builder->whereIn('status', ['awaiting', 'pending', 'confirmed', 'rescheduled', 'started']);
...
function build_chat_user_payload(
    ...
    // Resolve image URL using fix_provider_path (same logic as build_chat_message_details).
    $imageUrl = !empty($rawImage)
        ? fix_provider_path($rawImage)
        : base_url('public/backend/assets/profiles/default.png');
    if (empty($imageUrl)) {
        $imageUrl = base_url('public/backend/assets/profiles/default.png');
    }
...
function fetch_partner_formatted_data($user_id)
{
    ...
    foreach ($docFieldIds as $docId) {
        ...
        switch ($disk) {
            case 'local_server':
                $fullPath = FCPATH . ltrim($rawValue, '/');
                $partnerCustomFieldValues[$docId] = (!empty($rawValue) && is_file($fullPath))
                    ? base_url($rawValue)
                    : '';
                break;
            case 'aws_s3':
                $partnerCustomFieldValues[$docId] = !empty($rawValue)
                    ? fetch_cloud_front_url('custom_fields', $rawValue)
                    : '';
                break;
        }
    }
