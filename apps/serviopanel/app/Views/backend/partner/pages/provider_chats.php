                            ...
                            <div id="customer" class="">
                                <div id="customer-list">
                                    <?php foreach ($customers as $user) : ?>
                                        <div class="msg" onclick="setallMessage(<?= $user['id'] ?>, this, 'customer','<?= $user['order_id'] ?>')" data-customer-id="<?= $user['id'] ?>" data-order-id="<?= $user['order_id'] ?>">
                                            <?php
                                ...
                                <div class="ml-auto">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu <?= ($is_rtl == 1) ? 'dropdown-menu-left' : 'dropdown-menu-right' ?>" aria-labelledby="chatActionsDropdown" style="<?= ($is_rtl == 1) ? 'max-width: 100%; white-space: normal; word-wrap: break-word;' : '' ?>">
                                            ...
                                            <div class="dropdown-divider delete-chat-divider" style="display:none;"></div>
                                            <a class="dropdown-item text-danger delete-chat-option" href="#" onclick="deleteChat();" style="display:none; <?= ($is_rtl == 1) ? 'text-align: right;' : '' ?>">
                            ...
                            <div class="chat-area-footer d-none" style="display: flex; align-items: center;">
                                <form action="<?= base_url('admin/store_chat') ?>" method="post" style="flex: 1; display: flex; align-items: center;" enctype="multipart/form-data">
                                    <?php if ($enable_chat_image_upload == 1) : ?>
                                        ...
                                        <input id="imageInput" name="attachment[]" multiple type="file" accept="image/*" style="display: none; margin-right: 5px;" />
                                    <?php endif; ?>
                                    ...
                                    <?php if ($enable_chat_file_upload == 1) : ?>
                                        ...
                                        <input id="fileInput" name="attachment[]" multiple type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.odt,.ods,.odp,.rtf,.txt,.zip,.rar,.7z,.tar,.gz,.csv,.json,.xml,.mp4,.mov,.avi,.mkv,.wmv,.flv,.webm,.m4v,.3gp" style="display: none; margin-right: 5px;" />
                                    <?php endif; ?>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.rows && data.rows.length > 0 && parseInt(data.total) > 0) {
                        ...
                        if (data.receiver_profile_image) {
                            $('#receiver_user_profile').attr('src', data.receiver_profile_image);
                        }
                        if (order_id) {
